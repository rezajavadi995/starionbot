...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("crashed_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    )
    op.create_index("ix_crash_rounds_runtime_round_id", "crash_rounds", ["runtime_round_id"], unique=True)
    op.create_index("ix_crash_rounds_state", "crash_rounds", ["state"], unique=False)

    op.create_table(
        "crash_bets",
        sa.Column("id", sa.Integer(), primary_key=True),
        sa.Column("round_id", sa.Integer(), sa.ForeignKey("crash_rounds.id", ondelete="CASCADE"), nullable=False),
