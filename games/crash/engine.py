...
class CrashEngine:
    ...
    def seed_round(self) -> CrashRound:
        ...
        return CrashRound(
            state=RoundState.WAITING, multiplier=Decimal("1.00"), crash_point=crash_point
        )
    ...
    def step(self, round_state: CrashRound) -> CrashRound:
        ...
        state = (
            RoundState.ACTIVE if new_multiplier < round_state.crash_point else RoundState.CRASHED
        )
        return CrashRound(
            state=state, multiplier=new_multiplier, crash_point=round_state.crash_point
        )
